package services

import (
	"goinfras-sample-account/core"
	"goinfras-sample-account/core/oauth2"
	"goinfras-sample-account/core/user"
	"goinfras-sample-account/core/verified"
	"github.com/bb-orz/goinfras/XGlobal"
	"github.com/bb-orz/goinfras/XOAuth"
	"github.com/bb-orz/goinfras/XValidate"
	"sync"
)

// 服务层，实现services包定义的服务并设置该服务的实例，
// 需在服务实现的方法中验证DTO传输参数并调用具体的领域层业务逻辑
// 接收领域层和dao层的错误包装处理

var _ IUserService = new(UserService)

func init() {
	// 初始化该业务模块时实例化服务
	var once sync.Once
	once.Do(func() {
		userService := new(UserService)
		userService.userDomain = user.NewUserDomain()
		SetUserService(userService)
	})
}

type UserService struct {
	userDomain     *user.UserDomain         // 用户操作相关领域逻辑
	oauthDomain    *oauth2.OauthDomain      // 第三方鉴权相关领域逻辑
	verifiedDomain *verified.VerifiedDomain // 邮箱、短信验证相关领域逻辑
}

// 邮箱创建用户账号
func (service *UserService) CreateUserWithEmail(dto CreateUserWithEmailDTO) (*UserDTO, error) {
	var err error
	var isExist bool
	var userDTO *UserDTO

	// 校验传输参数
	if err = XValidate.V(dto); err != nil {
		return nil, core.WrapError(err, core.ErrorFormatServiceDTOValidate)
	}

	// 验证用户邮箱是否存在
	if isExist, err = service.userDomain.IsEmailExist(dto.Email); err != nil {
		return nil, core.WrapError(err, core.ErrorFormatServiceStorage)
	} else if isExist {
		return nil, core.WrapError(err, core.ErrorFormatServiceCheckInfo, "该用户已经存在!")
	}

	userDTO, err = service.userDomain.CreateUserForEmail(dto)
	if err != nil {
		return nil, core.WrapError(err, core.ErrorFormatServiceBusinesslogic, "创建用户失败！")
	}
	return userDTO, nil
}

// 邮箱创建用户账号
func (service *UserService) CreateUserWithPhone(dto CreateUserWithPhoneDTO) (*UserDTO, error) {
	var err error
	var isExist bool
	var userDTO *UserDTO

	// 校验传输参数
	if err = XValidate.V(dto); err != nil {
		return nil, core.WrapError(err, core.ErrorFormatServiceDTOValidate)
	}

	// 验证用户邮箱是否存在
	if isExist, err = service.userDomain.IsPhoneExist(dto.Phone); err != nil {
		return nil, core.WrapError(err, core.ErrorFormatServiceStorage)
	} else if isExist {
		return nil, core.WrapError(err, core.ErrorFormatServiceCheckInfo, "该用户已经存在!")
	}

	userDTO, err = service.userDomain.CreateUserForPhone(dto)
	if err != nil {
		return nil, core.WrapError(err, core.ErrorFormatServiceBusinesslogic, "创建用户失败！")
	}
	return userDTO, nil
}

// 邮箱账号登录鉴权
func (service *UserService) EmailAuth(dto AuthWithEmailPasswordDTO) (string, error) {
	var err error
	var userDTO *UserDTO
	var token string

	// 校验传输参数
	if err = XValidate.V(dto); err != nil {
		return "", core.WrapError(err, core.ErrorFormatServiceDTOValidate)
	}

	// 查找邮件账号是否存在
	if userDTO, err = service.userDomain.GetUserInfoByEmail(dto.Email); err != nil {
		return "", core.WrapError(err, core.ErrorFormatServiceStorage)
	}
	if userDTO == nil {
		return "", core.WrapError(err, core.ErrorFormatServiceCheckInfo, "该用户不存在!")
	} else if !XGlobal.ValidatePassword(dto.Password, userDTO.Salt, userDTO.Password) {
		// 校验密码失败
		return "", core.WrapError(err, core.ErrorFormatServiceCheckInfo, "密码错误!")
	}

	// JWT token
	token, err = service.userDomain.GenToken(userDTO.No, userDTO.Name, userDTO.Avatar)
	if err != nil {
		return "", core.WrapError(err, core.ErrorFormatServiceBusinesslogic)
	}

	return token, nil
}

// 手机账号登录鉴权
func (service *UserService) PhoneAuth(dto AuthWithPhonePasswordDTO) (string, error) {
	var err error
	var userDTO *UserDTO
	var token string

	// 校验传输参数
	if err = XValidate.V(dto); err != nil {
		return "", core.WrapError(err, core.ErrorFormatServiceDTOValidate)
	}

	// 查找手机账号是否存在
	userDTO, err = service.userDomain.GetUserInfoByPhone(dto.Phone)
	if err != nil {
		return "", core.WrapError(err, core.ErrorFormatServiceStorage)
	}

	if userDTO == nil {
		return "", core.WrapError(err, core.ErrorFormatServiceCheckInfo, "该用户不存在!")
	} else if !XGlobal.ValidatePassword(dto.Password, userDTO.Salt, userDTO.Password) {
		// 校验密码失败
		return "", core.WrapError(err, core.ErrorFormatServiceCheckInfo, "密码错误!")
	}

	// JWT token
	token, err = service.userDomain.GenToken(userDTO.No, userDTO.Name, userDTO.Avatar)
	if err != nil {
		return "", core.WrapError(err, core.ErrorFormatServiceBusinesslogic)
	}
	return token, nil
}

// 获取用户信息
func (service *UserService) GetUserInfo(dto GetUserInfoDTO) (*UserDTO, error) {
	var err error
	var userDTO *UserDTO

	// 校验传输参数
	if err = XValidate.V(dto); err != nil {
		return nil, core.WrapError(err, core.ErrorFormatServiceDTOValidate)
	}

	// 查找用户信息
	userDTO, err = service.userDomain.GetUserInfo(dto.ID)
	if err != nil {
		return nil, core.WrapError(err, core.ErrorFormatServiceStorage)
	}

	return userDTO, nil
}

// 批量设置用户信息
func (service *UserService) SetUserInfos(dto SetUserInfoDTO) error {
	var err error

	// 校验传输参数
	if err = XValidate.V(dto); err != nil {
		return core.WrapError(err, core.ErrorFormatServiceDTOValidate)
	}

	uid := dto.ID
	err = service.userDomain.SetUserInfos(uid, dto)
	if err != nil {
		return core.WrapError(err, core.ErrorFormatServiceStorage)
	}

	return nil
}

// 验证用户邮箱
func (service *UserService) ValidateEmail(dto ValidateEmailDTO) (bool, error) {
	var err error
	var pass bool

	// 校验传输参数
	if err = XValidate.V(dto); err != nil {
		return false, core.WrapError(err, core.ErrorFormatServiceDTOValidate)
	}

	// 从cache拿出保存的邮箱验证码
	pass, err = service.verifiedDomain.VerifiedEmail(dto.ID, dto.VerifiedCode)
	if err != nil {
		return false, core.WrapError(err, core.ErrorFormatServiceCache, "缓存验证码校验错误")
	}

	if pass {
		return true, nil
	}

	return false, nil
}

// 验证手机号码
func (service *UserService) ValidatePhone(dto ValidatePhoneDTO) (bool, error) {
	var err error
	var pass bool

	// 校验传输参数
	if err = XValidate.V(dto); err != nil {
		return false, core.WrapError(err, core.ErrorFormatServiceDTOValidate)
	}

	// 从cache拿出保存的短信验证码
	pass, err = service.verifiedDomain.VerifiedPhone(dto.ID, dto.VerifiedCode)
	if err != nil {
		return false, core.WrapError(err, core.ErrorFormatServiceCache, "缓存验证码校验错误")
	}

	if pass {
		return true, nil
	}

	return false, nil
}

// 设置用户账号状态
func (service *UserService) SetStatus(dto SetStatusDTO) (int, error) {
	var err error
	// 校验传输参数
	if err = XValidate.V(dto); err != nil {
		return -1, core.WrapError(err, core.ErrorFormatServiceDTOValidate)
	}

	err = service.userDomain.SetStatus(dto.ID, dto.Status)
	if err != nil {
		return -1, core.WrapError(err, core.ErrorFormatServiceStorage)
	}

	return 0, nil
}

// 修改用户密码
func (service *UserService) ChangePassword(dto ChangePasswordDTO) error {
	var err error
	var userDTO *UserDTO

	// 校验传输参数
	if err = XValidate.V(dto); err != nil {
		return core.WrapError(err, core.ErrorFormatServiceDTOValidate)
	}

	// 查找账号是否存在
	userDTO, err = service.userDomain.GetUserInfo(dto.ID)
	if err != nil {
		return core.WrapError(err, core.ErrorFormatServiceStorage)
	}

	// 校验旧密码
	if userDTO == nil {
		return core.WrapError(err, core.ErrorFormatServiceCheckInfo, "该用户不存在!")
	} else if !XGlobal.ValidatePassword(dto.Old, userDTO.Salt, userDTO.Password) {
		// 校验旧密码失败
		return core.WrapError(err, core.ErrorFormatServiceCheckInfo, "旧密码错误!")
	}

	// 设置新密码
	if err = service.userDomain.ReSetPassword(dto.ID, dto.New); err != nil {
		return core.WrapError(err, core.ErrorFormatServiceStorage)
	}

	return nil
}

// 忘记密码重设
func (service *UserService) ForgetPassword(dto ForgetPasswordDTO) error {
	var err error
	var isExist bool
	var isVerified bool

	// 校验传输参数
	if err = XValidate.V(dto); err != nil {
		return core.WrapError(err, core.ErrorFormatServiceDTOValidate)
	}

	// 查找账号是否存在
	isExist, err = service.userDomain.IsUserExist(dto.ID)
	if err != nil {
		return core.WrapError(err, core.ErrorFormatServiceStorage)
	}
	if !isExist {
		return core.WrapError(err, core.ErrorFormatServiceCheckInfo, "该用户不存在!")
	}

	// 校验Code
	isVerified, err = service.verifiedDomain.VerifiedResetPasswordCode(dto.ID, dto.Code)
	if err != nil {
		return core.WrapError(err, core.ErrorFormatServiceCache)
	}

	if !isVerified {
		return core.WrapError(nil, core.ErrorFormatServiceCheckInfo, "重置密码校验码错误，请重试！")
	}

	return nil

}

// 上传用户头像
func (service *UserService) UploadAvatar() error {

	return nil
}

// qq oauth 鉴权
func (service *UserService) QQOAuth(dto QQLoginDTO) (string, error) {
	var err error
	var token string
	var qqOauthAccountInfo *XOAuth.OAuthAccountInfo // qq账号鉴权信息
	var findUserBindingDTO *UserOAuthsDTO  // 查找绑定用户
	var userOAuthsInfo *UserOAuthsDTO      // 创建用户后的信息

	// 校验传输参数
	if err = XValidate.V(dto); err != nil {
		return "", core.WrapError(err, core.ErrorFormatServiceDTOValidate)
	}

	// oauth domain：使用qq回调授权码code开始鉴权流程并获取QQ用户信息
	qqOauthAccountInfo, err = service.oauthDomain.GetQQOauthUserInfo(dto.AccessCode)
	if err != nil {
		return "", core.WrapError(err, core.ErrorFormatServiceNetRequest, "GetQQUserInfo")
	}

	// oauth domain: 使用OpenId UnionId查找user oauth表查看用户是否存在
	findUserBindingDTO, err = service.userDomain.GetUserOauths(user.QQOauthPlatform, qqOauthAccountInfo.OpenId, qqOauthAccountInfo.UnionId)
	if err != nil {
		return "", core.WrapError(err, core.ErrorFormatServiceStorage, "IsOauthUserExist")
	}

	// 如不存在进入创建用户流程,否则进登录流程
	if findUserBindingDTO == nil {
		userOAuthsInfo, err = service.userDomain.CreateUserOAuthBinding(user.QQOauthPlatform, qqOauthAccountInfo)
		// JWT token
		if userOAuthsInfo != nil {
			token, err = service.userDomain.GenToken(
				userOAuthsInfo.User.No,
				userOAuthsInfo.User.Name,
				userOAuthsInfo.User.Avatar)
			if err != nil {
				return "", core.WrapError(err, core.ErrorFormatServiceBusinesslogic)
			}
			return token, nil
		} else {
			return "", core.WrapError(err, core.ErrorFormatServiceStorage, "CreateUserOauthBinding")
		}
	}

	// 跳过创建，直接返回token，登录成功
	token, err = service.userDomain.GenToken(
		findUserBindingDTO.User.No,
		findUserBindingDTO.User.Name,
		findUserBindingDTO.User.Avatar)
	if err != nil {
		return "", core.WrapError(err, core.ErrorFormatServiceBusinesslogic)
	}

	return token, nil
}

// wechat Oauth 鉴权
func (service *UserService) WechatOAuth(dto WechatLoginDTO) (string, error) {
	var err error
	var token string
	var wechatOauthAccountInfo *XOAuth.OAuthAccountInfo // 微信账号鉴权信息
	var findUserBindingDTO *UserOAuthsDTO      // 查找绑定用户
	var userOAuthsInfo *UserOAuthsDTO          // 创建用户后的信息

	// 校验传输参数
	if err = XValidate.V(dto); err != nil {
		return "", core.WrapError(err, core.ErrorFormatServiceDTOValidate)
	}

	// oauth domain：使用wechat回调授权码code开始鉴权流程并获取微信用户信息
	wechatOauthAccountInfo, err = service.oauthDomain.GetQQOauthUserInfo(dto.AccessCode)
	if err != nil {
		return "", core.WrapError(err, core.ErrorFormatServiceNetRequest, "GetQQUserInfo")
	}

	// oauth domain: 使用OpenId UnionId查找user oauth表查看用户是否存在
	findUserBindingDTO, err = service.userDomain.GetUserOauths(user.WechatOauthPlatform, wechatOauthAccountInfo.OpenId, wechatOauthAccountInfo.UnionId)
	if err != nil {
		return "", core.WrapError(err, core.ErrorFormatServiceStorage, "IsOauthUserExist")
	}

	// 如不存在进入创建用户流程,否则进登录流程
	if findUserBindingDTO == nil {
		userOAuthsInfo, err = service.userDomain.CreateUserOAuthBinding(user.WechatOauthPlatform, wechatOauthAccountInfo)
		// JWT token
		if userOAuthsInfo != nil {
			token, err = service.userDomain.GenToken(
				userOAuthsInfo.User.No,
				userOAuthsInfo.User.Name,
				userOAuthsInfo.User.Avatar)
			if err != nil {
				return "", core.WrapError(err, core.ErrorFormatServiceBusinesslogic)
			}
			return token, nil
		} else {
			return "", core.WrapError(err, core.ErrorFormatServiceStorage, "CreateUserOauthBinding")
		}
	}

	// 跳过创建，直接返回token，登录成功
	token, err = service.userDomain.GenToken(
		findUserBindingDTO.User.No,
		findUserBindingDTO.User.Name,
		findUserBindingDTO.User.Avatar)
	if err != nil {
		return "", core.WrapError(err, core.ErrorFormatServiceBusinesslogic)
	}

	return token, nil
}

// 微博 Oauth 鉴权
func (service *UserService) WeiboOAuth(dto WeiboLoginDTO) (string, error) {
	var err error
	var token string
	var weiboOauthAccountInfo *XOAuth.OAuthAccountInfo // 微博账号鉴权信息
	var findUserBindingDTO *UserOAuthsDTO     // 查找绑定用户
	var userOAuthsInfo *UserOAuthsDTO         // 创建用户后的信息

	// 校验传输参数
	if err = XValidate.V(dto); err != nil {
		return "", core.WrapError(err, core.ErrorFormatServiceDTOValidate)
	}

	// oauth domain：使用wechat回调授权码code开始鉴权流程并获取微信用户信息
	weiboOauthAccountInfo, err = service.oauthDomain.GetQQOauthUserInfo(dto.AccessCode)
	if err != nil {
		return "", core.WrapError(err, core.ErrorFormatServiceNetRequest, "GetQQUserInfo")
	}

	// oauth domain: 使用OpenId UnionId查找user oauth表查看用户是否存在
	findUserBindingDTO, err = service.userDomain.GetUserOauths(user.WeiboOauthPlatform, weiboOauthAccountInfo.OpenId, weiboOauthAccountInfo.UnionId)
	if err != nil {
		return "", core.WrapError(err, core.ErrorFormatServiceStorage, "IsOauthUserExist")
	}

	// 如不存在进入创建用户流程,否则进登录流程
	if findUserBindingDTO == nil {
		userOAuthsInfo, err = service.userDomain.CreateUserOAuthBinding(user.WeiboOauthPlatform, weiboOauthAccountInfo)
		// JWT token
		if userOAuthsInfo != nil {
			token, err = service.userDomain.GenToken(
				userOAuthsInfo.User.No,
				userOAuthsInfo.User.Name,
				userOAuthsInfo.User.Avatar)
			if err != nil {
				return "", core.WrapError(err, core.ErrorFormatServiceBusinesslogic)
			}
			return token, nil
		} else {
			return "", core.WrapError(err, core.ErrorFormatServiceStorage, "CreateUserOauthBinding")
		}
	}

	// 跳过创建，直接返回token，登录成功
	token, err = service.userDomain.GenToken(
		findUserBindingDTO.User.No,
		findUserBindingDTO.User.Name,
		findUserBindingDTO.User.Avatar)
	if err != nil {
		return "", core.WrapError(err, core.ErrorFormatServiceBusinesslogic)
	}

	return token, nil
}
